...
void main(void) {
    ...
    float marker_size = sqrt(size) * marker_scale * 4.;
...
// #endif
    ...
    // gl_Position = vec4(rotate_xy(position, angle) * full_size, 1.0) + vec4(center_pixels + vec3(0., 0., 0.), 1.0);
    gl_Position = vec4(position * full_size, 1.0) + vec4(center_pixels, 1.0);
}
